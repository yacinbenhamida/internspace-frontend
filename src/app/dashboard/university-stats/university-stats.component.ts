...
export class UniversityStatsComponent implements OnInit, AfterViewInit {
  ...
  chart2() {
    ...
    const chart = am4core.create('chart2', am4maps.MapChart);

    const title = chart.titles.create();
    title.text = '[bold font-size: 20]Students Distribution[/]\nsource: Gapminder';
    ...
    const mapData = [
      ...
      { "id": "TN", "name": "Tunisia", "latitude": 50, "longitude": 50, "value": 10594057, "color": chart.colors.getIndex(2) },
    ...
    console.log(circle.configField);
    circle.propertyFields.fill = "color";
    circle.tooltipText = "{name}: [bold]{value}[/]";

    circle.cursorDownStyle = am4core.MouseCursorStyle.grabbing;
    circle.cursorOverStyle = am4core.MouseCursorStyle.pointer;

    circle.events.on('down', (ev) => { console.log(ev.target.dataItem) });
    circle.events.on("hit", (ev) => { console.log(ev) });
